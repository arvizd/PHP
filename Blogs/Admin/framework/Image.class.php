...
// define('VERIFY_LENGTH',5);
// define('VERIFY_MODE',1);
// define('VERIFY_TYPE','png');
// define('VERIFY_WIDTH',56);
// define('VERIFY_HEIGHT',22);
class Image {
    ...
    static function buildImageVerify($length=NULL, $mode=NULL, $type=NULL, $width=NULL, $height=NULL, $verifyName='verify') {
       // import('ORG.Util.String');
        $length=empty($length)?C('VERIFY_LENGTH'):$length;
        $mode=empty($mode)?C('VERIFY_MODE'):$mode;
        $type=empty($type)?C('VERIFY_TYPE'):$type;
        $width=empty($width)?C('VERIFY_WIDTH'):$width;
        $height=empty($height)?C('VERIFY_HEIGHT'):$height;
        require './Blogs/Admin/framework/String.class.php';
